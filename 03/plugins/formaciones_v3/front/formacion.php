...
echo "<th>" . __('Tipo', 'formaciones') . "</th>";
echo "<th>" . __('Instructor', 'formaciones') . "</th>";
...
foreach ($rows as $row) {
    ...
    // Obtiene el nombre visible del desplegable Tipo - Formaciones.
    $type_name = '';
    if (!empty($row['plugin_formaciones_tipoformacions_id'])) {
        $type_name = Dropdown::getDropdownName(
            PluginFormacionesTipoFormacion::getTable(),
            (int) $row['plugin_formaciones_tipoformacions_id']
        );
    }

    // Carga el instructor relacionado para mostrar "Nombre Apellidos".
    $instructor_name = '';
    if (!empty($row['plugin_formaciones_instructors_id'])) {
        $instructor = new PluginFormacionesInstructor();
        if ($instructor->getFromDB((int) $row['plugin_formaciones_instructors_id'])) {
            $instructor_name = $instructor->getName();
        }
    }
    ...
    // Muestra los valores relacionados ya convertidos a texto legible.
    echo "<td>" . htmlescape($type_name) . "</td>";
    echo "<td>" . htmlescape($instructor_name) . "</td>";
