...
class PluginFormacionesFormacion extends CommonDBTM
{
    ...
    private function cleanInput(array $input)
    {
        ...
        // Convierte a entero las claves externas recibidas desde los desplegables.
        foreach (['plugin_formaciones_tipoformacions_id', 'plugin_formaciones_instructors_id'] as $field) {
    ...
    public function showForm($ID, array $options = [])
    {
        ...
        // Segunda fila: tipo e instructor.
        echo '<tr class="tab_bg_1">';
        echo '<td>' . __('Tipo', 'formaciones') . '</td>';
        echo '<td>';

        // Desplegable propio mantenible desde Configuracion > Desplegables.
        $type_dropdown = Dropdown::show(PluginFormacionesTipoFormacion::class, [
            'name'  => 'plugin_formaciones_tipoformacions_id',
            'value' => $this->fields['plugin_formaciones_tipoformacions_id'] ?? 0
        ]);
        if ($type_dropdown === false) {
            Dropdown::showFromArray(
                'plugin_formaciones_tipoformacions_id',
                PluginFormacionesTipoFormacion::getDropdownOptions(),
                [
                    'value'               => $this->fields['plugin_formaciones_tipoformacions_id'] ?? 0,
        ...
        // Selector de Instructor. Usamos opciones directas para evitar que el
        // ajax generico de GLPI trate este objeto propio como un desplegable
        // administrable y deje el listado vacio.
        Dropdown::showFromArray(
            'plugin_formaciones_instructors_id',
            PluginFormacionesInstructor::getDropdownOptions(),
            [
                'value'               => $this->fields['plugin_formaciones_instructors_id'] ?? 0,
                'display_emptychoice' => true
            ]
        );
        echo '</td>';
        ...
        // Tercera fila: descripcion larga.
        echo '<tr class="tab_bg_1">';
    ...
    public function rawSearchOptions()
    {
        ...
        // Columna Tipo - Formaciones.
        $tab[] = [
            'id'        => '7',
            // Tabla enlazada que contiene el nombre visible del desplegable.
            'table'     => PluginFormacionesTipoFormacion::getTable(),
            'field'     => 'name',
            // Campo de la tabla Formaciones que guarda el ID seleccionado.
            'linkfield' => 'plugin_formaciones_tipoformacions_id',
            'name'      => __('Tipo', 'formaciones'),
            'itemtype'  => PluginFormacionesTipoFormacion::class,
            'datatype'  => 'dropdown'
        ];
        ...
        $tab[] = [
            'id'        => '8',
            // Tabla enlazada que contiene los datos del instructor.
            'table'     => PluginFormacionesInstructor::getTable(),
            'field'     => 'name',
            // Campo de la tabla Formaciones que guarda el instructor seleccionado.
            'linkfield' => 'plugin_formaciones_instructors_id',
            'name'      => __('Instructor', 'formaciones'),
            'itemtype'  => PluginFormacionesInstructor::class,
            'datatype'  => 'dropdown'
        ];
