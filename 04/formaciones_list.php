<?php
    ...
    /**
     * Ejemplo formativo: recuperar formaciones de GLPI usando guzzlehttp/guzzle.
     *
     * Este ejemplo consume un objeto creado por un plugin de GLPI:
    ...
    {
        // El plugin guarda el estado como 0/1. Esta tabla lo traduce a texto.
        $states = [
            0 => 'Inactivo',
            1 => 'Activo',
        ];
    ...
    // El flujo de API solo se ejecuta al pulsar el boton Recuperar formaciones.
    if (isset($_POST['load_formaciones'])) {
        if (!$hasGuzzle) {
            $messages[] = [
                'type' => 'warning',
            ...
            try {
                ...
                // 3) Consultamos el itemtype del plugin de formaciones.
                $formacionesResponse = $client->get('PluginFormacionesFormacion/', [
                    'headers' => $headers,
                    'query'   => [
                        'range'            => '0-49',
                ...
                $jsonOutput = (string) $formacionesResponse->getBody();

                // Convertimos el JSON a array para pintar la tabla.
                $formaciones = json_decode($jsonOutput, true) ?: [];

                // Re-formateamos el JSON para que sea legible en pantalla.
                $jsonOutput = json_encode($formaciones, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                ...
                $messages[] = [
                    'type' => 'success',
                    'text' => 'Listado de formaciones recuperado correctamente.'
                ];
...
<head>
    ...
    <title>PHP + GLPI 11 | Listado de formaciones con Guzzle</title>
    ...
    <div class="layout d-flex">
        ...
        <main class="flex-grow-1 p-4 p-lg-5">
            <div class="container-fluid">
                <header class="mb-4">
                    <h1 class="display-6 fw-bold mb-3">Listado de formaciones con Guzzle</h1>
                ...
                <section class="welcome-panel bg-white p-4 p-lg-5 shadow-sm">
                    <div class="mx-auto" style="max-width: 1180px;">
                        <div class="text-center mb-5">
                            <h2 class="fw-bold mb-3">Recuperar formaciones con guzzlehttp/guzzle</h2>
                            <p class="lead text-secondary mb-0">
                                Este ejemplo abre sesion en GLPI, consulta el itemtype
                                <code>PluginFormacionesFormacion</code>, muestra el JSON recibido y
                                despues presenta una tabla simplificada.
                            </p>
                        </div>
                            ...
                            <div class="col-lg-5">
                                <div class="border rounded-3 p-4 h-100">
                                    ...
                                    <form method="post">
                                        ...
                                        <button class="btn btn-primary mt-4" type="submit" name="load_formaciones" value="1">
                                            Recuperar formaciones
                                        </button>
                                    </form>
                                </div>
                            </div>
...
$response = $client-&gt;get('PluginFormacionesFormacion/', [
    ...
    'query' =&gt; [
        ...
        'expand_dropdowns' =&gt; 'true',
                        ...
                        <section>
                            <h3 class="h4 fw-bold mb-3">Tabla de formaciones</h3>
                                        ...
                                        <?php if (count($formaciones) === 0): ?>
                                            <tr>
                                                <td colspan="4" class="text-center text-secondary">
                                                    No hay datos para mostrar. Pulsa <strong>Recuperar formaciones</strong>.
                                                </td>
                                            </tr>
                                        <?php else: ?>
